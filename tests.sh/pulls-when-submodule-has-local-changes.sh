...
cd sub || exit 1
echo 'upstream update' > pom.xml
git commit --no-verify -am "upstream change"
cd .. || exit 1

cd foo || exit 1
echo 'LOCAL ACCIDENTAL CHANGE' > sub/pom.xml

env "GITTED_TESTING=true" "GIT_ALLOW_PROTOCOL=file" \
    "${base}/scripts/pull" 2>&1 | tee "${tmp}/log.txt"

grep 'upstream update' sub/pom.xml
! grep -q 'LOCAL ACCIDENTAL CHANGE' sub/pom.xml
